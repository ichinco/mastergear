...
<div class="fieldcontain ${hasErrors(bean: gearListInstance, field: 'hikeType', 'error')} required">
    ...
    <g:select name="hikeType" from="${com.mastergear.HikeType?.values()}"
              keys="${com.mastergear.HikeType.values()*.name()}" required=""
              value="${gearListInstance?.hikeType?.name()}"/>
</div>
...
<div class="fieldcontain ${hasErrors(bean: gearListInstance, field: 'season', 'error')} required">
    ...
    <g:select name="season" from="${com.mastergear.Season?.values()}" keys="${com.mastergear.Season.values()*.name()}"
              required="" value="${gearListInstance?.season?.name()}"/>
</div>
...
<div class="fieldcontain ${hasErrors(bean: gearListInstance, field: 'trail', 'error')} required">
    ...
    <g:select id="trail" name="trail.id" from="${com.mastergear.Trail.list()}" optionKey="id" required=""
              value="${gearListInstance?.trail?.id}" class="many-to-one"/>
</div>
